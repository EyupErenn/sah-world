    ...
    if (localData.journal?.length) {
      await supabase.from('journal_entries').upsert(
        localData.journal.map((j) => ({
          ...
          ritual_type: j.ritualType ?? null,
          entry_mode: j.entryMode ?? 'full',
          niyet_text: j.intentionText ?? '',
          beklenen_zorluk_text: j.expectedChallengeText ?? '',
          gratitude_text: j.gratitudeText ?? '',
          xp_awarded: j.xpAwarded ?? 0,
          tags: j.tags ?? [],
    ...
    async function loadAllData() {
      ...
      try {
        ...
        const journal: JournalEntry[] = (journalData || []).map((j) => ({
          ...
          ritualType: j.ritual_type ?? null,
          entryMode: j.entry_mode ?? 'full',
          intentionText: j.niyet_text ?? '',
          expectedChallengeText: j.beklenen_zorluk_text ?? '',
          gratitudeText: j.gratitude_text ?? '',
