...
export function AdminFeedbackClient({
  ...
  const pageHref = (page: number) => {
    ...
    return `/admin/feedback?${next}`;
  };

  return (
    <>
      <header className="admin-header">
        <Link className="standalone-brand" href="/">
          <span>S</span>
          <strong>SAH</strong>
        </Link>
        <div>
          <span>Yönetim Merkezi</span>
          <Link href="/feedback">Kullanıcı görünümü</Link>
        </div>
      </header>
      <div className="admin-shell">
        <div className="admin-heading">
          <div>
            <p className="eyebrow">Ürün zekâsı</p>
            <h1>Geri Bildirim Merkezi</h1>
            <p>
              Kullanıcı sinyallerini tek kuyrukta değerlendir, yanıtla ve ürün
              kararlarına dönüştür.
            </p>
          </div>
          <span className="admin-secure-chip">✓ Sunucu + RLS korumalı</span>
        </div>
        <section className="admin-stats">
          <AdminStat
            ...
            icon="◎"
          />
          <AdminStat label="Yeni" value={stats?.received_count || 0} icon="✦" />
          <AdminStat
            label="İnceleniyor"
            value={stats?.reviewing_count || 0}
            icon="⌁"
          />
          <AdminStat
            label="Planlandı"
        ...
        <section className="admin-list surface-card">
          ...
          {items.length === 0 ? (
            <div className="empty-state">
              ...
              <p>
                Filtreleri temizleyebilir veya yeni gönderimleri
                bekleyebilirsin.
              </p>
            </div>
          ) : (
            items.map((item) => (
              <details className="admin-feedback-item" key={item.id}>
                <summary>
                  <AvatarImage
                    src={
                      item.avatar_url ||
                      `https://api.dicebear.com/9.x/initials/svg?seed=${encodeURIComponent(item.display_name)}`
                    }
                    alt=""
                    size={44}
                  />
                  <div>
                    ...
                    <span>
                      {item.display_name} · {typeLabels[item.type]}
                    </span>
                  </div>
                      ...
                      {item.reviewed_at && (
                        <div>
                          ...
                          <dd>
                            {new Intl.DateTimeFormat("tr-TR", {
                              dateStyle: "medium",
                            }).format(new Date(item.reviewed_at))}
                          </dd>
                        </div>
                      )}
                    </dl>
                  ...
                  >
                    ...
                    <label>
                      ...
                      <textarea
                        ...
                        placeholder="Kararı ve sonraki adımı açıkça paylaş…"
                      />
                    </label>
                    <div>
                      <button name="archive" value="false" type="submit">
                        Kaydet ve yanıtla
                      </button>
                      <button
