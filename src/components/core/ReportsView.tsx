...
export default function ReportsView() {
  ...
  return <div className="view-stack reports-view"><header className="page-heading"><div><span className="eyebrow">GELİŞİM İÇGÖRÜLERİ</span><h1>Raporlarım</h1><p>Rakamları yargı için değil, ritmini anlamak ve dengen için kullan.</p></div><span className="quiet-chip">Son güncelleme · şimdi</span></header>
    <div className="stat-grid report-stats">
      <ReportStat icon="◎" label="Toplam XP" value={store.xp.toLocaleString('tr-TR')}/><ReportStat icon="↗" label="Bu hafta kayıt" value={weekCount}/><ReportStat icon="●" label="Bugünün kaydı" value={daily.get(today)??0}/><ReportStat icon="Σ" label="Toplam kayıt" value={total}/><ReportStat icon="⚡" label="En iyi seri" value={`${bestStreak} gün`}/><ReportStat icon="▦" label="Bu ay aktif gün" value={monthActive}/>
    </div>
    ...
    <section className="surface-card goals-card"><div className="card-heading"><div><span className="eyebrow">ÖNCELİKLER</span><h2>Hedef tamamlama</h2></div><strong className="completion-total">{tasks.length ? Math.round(tasks.filter(t=>t.done).length/tasks.length*100):0}%</strong></div>{tasks.length===0?<div className="empty-state compact"><i>⊞</i><strong>Henüz görev yok</strong><p>Öncelik Matrisinde ilk görevini eklediğinde tamamlanma dengesi burada görünür.</p></div>:<div className="quadrant-progress">{(['q1','q2','q3','q4'] as const).map((q,i)=>{const list=store.eisenhower[q], done=list.filter(t=>t.done).length, pct=list.length?done/list.length*100:0;return <article key={q}><span><b>Q{i+1}</b>{['Acil + Önemli','Önemli','Devredilebilir','Sadeleştir'][i]}</span><div><i style={{width:`${pct}%`}}/></div><strong>{done}/{list.length}</strong></article>})}</div>}</section>
  </div>;
}

function ReportStat({icon,label,value}:{icon:string;label:string;value:string|number}){return <article className="stat-card report-stat"><span className="stat-icon indigo">{icon}</span><div><small>{label}</small><strong>{value}</strong></div></article>}
...
function Heatmap({daily,range}:{daily:Map<string,number>;range:'month'|'year'}){const now=new Date(); const days=range==='month'?new Date(now.getFullYear(),now.getMonth()+1,0).getDate():365; const start=range==='month'?new Date(now.getFullYear(),now.getMonth(),1):new Date(now.getFullYear(),0,1); const cells=Array.from({length:days},(_,i)=>{const d=new Date(start);d.setDate(start.getDate()+i);const count=d>now?0:(daily.get(dayKey(d))??0);return {d,count,future:d>now}}); const max=Math.max(1,...cells.map(c=>c.count));return <div className={`heatmap ${range}`}>{cells.map(({d,count,future})=>{const level=count===0?0:Math.min(4,Math.ceil(count/max*4));return <span key={dayKey(d)} className={`heat-${level} ${future?'future':''}`} title={`${d.toLocaleDateString('tr-TR')}: ${count} kayıt`}><b>{range==='month'?d.getDate():''}</b></span>})}</div>}
