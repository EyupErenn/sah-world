...
const avatar = (name: string, url?: string | null) => {
  if (url) {
    try {
      ...
      // Invalid or untrusted remote avatar URLs use a deterministic safe fallback.
    }
  }
...
export default function QuranCompanionView({
  ...
  const load = async () => {
    ...
    setError(
      firstError
        ? "Kur’an Kardeşim verileri tamamen yüklenemedi. Lütfen tekrar dene."
        : "",
    );
    setLoading(false);
  };
  ...
  const saveLevel = async (level: QuranLevel) => {
    if (!isRealUser) {
      ...
      flash("Geliştirme görünümünde seviye seçildi.");
      return;
    }
  ...
  const navItems: Array<{ id: CompanionTab; label: string; icon: string }> = [
    { id: "home", label: "Başlangıç", icon: "home-heart" },
    { id: "wheel", label: "Günün Çarkı", icon: "refresh" },
    { id: "teachers", label: "Hocalar", icon: "calendar-user" },
    { id: "appointments", label: "Randevularım", icon: "calendar-check" },
    { id: "peers", label: "Akran desteği", icon: "heart-handshake" },
    ...
    <div className="quran-companion">
      ...
      <section className="quran-companion-hero">
        ...
        <div>
          ...
          <h1>Öğrenirken yalnız değilsin.</h1>
          <p>
            Güvenilir bir hocayla buluş, bir kardeşinle birlikte çalış ve Kur’an
            yolculuğunu küçük, istikrarlı adımlarla sürdür.
          </p>
          <SectionTagline section="quran-companion" compact inverse />
          <div className="quran-hero-actions">
            <button onClick={() => setTab("teachers")}>
              <AppIcon name="calendar-user" /> Hoca ile randevu al
            </button>
            <button onClick={() => setTab("study")}>
              <AppIcon name="notebook" /> Çalışma alanıma git
            </button>
          </div>
        </div>
        <aside>
          ...
          {upcoming[0] ? (
            ...
            <>
              <strong>Takvimin açık</strong>
              <span>Sana uygun bir hoca ve saat seçebilirsin.</span>
            </>
          )}
          <button
            onClick={() => setTab(upcoming[0] ? "appointments" : "teachers")}
          >
            {upcoming[0] ? "Randevuyu gör" : "Uygun saatleri gör"}{" "}
      ...
      <section className="quran-virtues" aria-labelledby="quran-virtue-title">
        <article className="quran-virtue-feature">
          <span className="eyebrow">ÖĞRENMENİN VE ÖĞRETMENİN FAZİLETİ</span>
          <p lang="ar" dir="rtl">
            خَيْرُكُمْ مَنْ تَعَلَّمَ الْقُرْآنَ وَعَلَّمَهُ
          </p>
          <h2 id="quran-virtue-title">
          ...
          <a
            ...
            rel="noreferrer"
          >
            Buhârî, Fedâilü’l-Kur’ân, 21; hadis no. 5027{" "}
            <AppIcon name="external-link" />
          </a>
        </article>
        <article className="quran-virtue-ayah">
          <span>
            <AppIcon name="book-2" />
          </span>
          <div>
            <small>FÂTIR 35:29-30 · KUR’AN YOLU MEALİ</small>
            <p>
              Allah’ın kitabını okuyanlar, namazı özenle kılanlar ve kendilerine
              verilen rızıktan paylaşanlar, asla zararla sonuçlanmayacak bir
              kazanç umabilirler.
            </p>
            <a
              href="https://kuran.diyanet.gov.tr/tefsir/fatir-suresi/3689/29-30-ayet-tefsiri"
              target="_blank"
              rel="noreferrer"
            >
              Meali ve bağlamı Diyanet’te incele{" "}
              <AppIcon name="external-link" />
            </a>
          </div>
        </article>
      ...
      {loading ? (
        ...
        <CompanionHome
          upcoming={upcoming}
          goal={goal}
          noteCount={journey.quranNotes.length}
          onTab={setTab}
        />
      ) : tab === "wheel" ? (
        <DailyWisdomWheel
          key={wheelEntry?.nonce ?? "wheel"}
          entry={wheelEntry}
        />
      ) : tab === "teachers" ? (
        <TeacherDiscovery
          ...
          onBooked={async () => {
            await load();
            setTab("appointments");
            flash("Randevun onaylandı ve takvimine eklendi.");
          }}
          realUser={isRealUser}
        />
      ) : tab === "appointments" ? (
        ...
        <StudyWorkspace
          ...
          onSaved={async () => {
            await load();
            flash("Çalışma hedefin güncellendi.");
          }}
        />
      ) : (
        <HocaManagement
...
function QuranInvitationVideo() {
  ...
  return (
    <section
      className="quran-invitation-video"
      aria-labelledby="quran-invitation-title"
    >
      <video
        ref={videoRef}
      ...
      <div className="quran-invitation-copy">
        ...
        <h2 id="quran-invitation-title">
          Genç Gel,
          <br />
          <em>Geç Gelme.</em>
        </h2>
        <p>
          Birlikte öğrenmek, güzel okumak ve Kur’an’la bağını istikrarlı
          adımlarla güçlendirmek için.
        </p>
      </div>
      <button
        className="quran-video-control"
        type="button"
        onClick={togglePlayback}
        aria-label={
          playing ? "Davet videosunu duraklat" : "Davet videosunu oynat"
        }
      >
        <AppIcon name={playing ? "player-pause" : "player-play"} />
        <span>{playing ? "Duraklat" : "Oynat"}</span>
      </button>
      <div className="quran-video-duration" aria-hidden="true">
        <AppIcon name="clock" /> 8 sn
      </div>
    </section>
  );
}
...
function CompanionHome({
  ...
  return (
    <section className="quran-home-grid">
      <button
        onClick={() => onTab("teachers")}
        className="quran-feature-card teacher"
      >
        <span>
          <AppIcon name="calendar-user" />
        </span>
        <small>REHBERLİK</small>
        <strong>Bir hocayla çalış</strong>
        <p>Uygun günü ve saati seç; randevun anında takvimine işlensin.</p>
        <em>
      ...
      <button
        onClick={() => onTab("peers")}
        className="quran-feature-card peer"
      >
        <span>
          <AppIcon name="heart-handshake" />
        </span>
        <small>BİRLİKTE ÖĞREN</small>
        <strong>Akran desteği bul</strong>
        <p>
          Kur’an yolculuğunda sana gönüllü destek olabilecek bir kardeşle tanış.
        </p>
        <em>
          Eşleşmeleri gör <AppIcon name="arrow-right" />
        </em>
      </button>
      <article className="quran-home-summary">
        <header>
          <span>
            <AppIcon name="progress" />
          </span>
          <div>
            <small>ÇALIŞMA ALANIN</small>
            <h3>{goal?.title || "İlk çalışma hedefini belirle"}</h3>
          </div>
        </header>
        <div className="quran-goal-mini">
          <i>
            <b style={{ width: `${goal?.progress_percent || 0}%` }} />
          </i>
          <strong>%{goal?.progress_percent || 0}</strong>
        </div>
        <footer>
          <span>
            <b>{noteCount}</b> Kur’an notu
          </span>
          <span>
            <b>{upcoming.length}</b> yaklaşan randevu
          </span>
          <button onClick={() => onTab("study")}>Alanı aç</button>
        </footer>
      </article>
    </section>
  );
    ...
    if (bookingError) {
      setError(
        bookingError.message.includes("SLOT_UNAVAILABLE")
          ...
          : "Randevu oluşturulamadı. Lütfen tekrar dene.",
      );
      setBusy(false);
...
function PeerMatching({
  ...
  return (
    <section className="quran-panel">
      <header className="quran-panel-heading">
        <div>
          <span className="eyebrow">AKRAN DESTEĞİ</span>
  ...
  return (
    <div className="quran-modal-backdrop">
      <section
        ...
        role="dialog"
        aria-modal="true"
        aria-label={`${match.partner_name} ile mesajlaşma`}
      >
        <header>
          <AvatarImage
            src={avatar(match.partner_name, match.partner_avatar)}
...
function StudyWorkspace({
  ...
  const related = appointments
    .filter((item) => item.student_id === userId && item.topic_notes)
    .slice(0, 4);
    ...
    <section className="study-workspace">
      <article className="study-goal-card">
        <header>
          ...
          <div>
            <small>ÇALIŞMA HEDEFİ</small>
            <h2>İstikrarlı bir adım belirle</h2>
          </div>
        </header>
        <form onSubmit={(event) => void save(event)}>
          <label>
            ...
            <textarea
              ...
              placeholder="Örn. Haftada üç kez tecvid pratiği yapmak…"
            />
          </label>
          ...
          <button className="primary-button" disabled={!realUser}>
            <AppIcon name="device-floppy" /> Hedefi kaydet
          </button>
        </form>
      </article>
      <article className="study-notes-card">
        <header>
          <div>
            <small>AYNI VERİ, YENİ BAĞLAM</small>
            <h2>Son Kur’an notların</h2>
          </div>
          <button onClick={() => onNavigate("quran")}>
            Tümünü aç <AppIcon name="arrow-right" />
          </button>
        </header>
        {notes.length === 0 ? (
          <EmptyState
            ...
            text="Bugünün Çarkı veya not arşivinden ilk notunu ekleyebilirsin."
            compact
          />
        ) : (
          <div>
            {notes.slice(0, 4).map((note) => (
              <button key={note.id} onClick={() => onNavigate("quran")}>
                <span>
                  <AppIcon name="book-2" />
                </span>
                <div>
                  <strong>{note.ayet || note.sure}</strong>
                  <p>{note.ders || note.tefsir}</p>
                  <small>
      ...
      <article className="study-reflection-card">
        ...
        {related.length === 0 ? (
          <p>
            İlk hoca görüşmenden sonra çalışma başlıkların burada kısa bir
            hazırlık ve değerlendirme listesine dönüşecek.
          </p>
        ) : (
          <ul>
            {related.map((item) => (
              <li key={item.id}>
                ...
                <span>
                  <strong>{item.topic_notes}</strong>
                  <small>
                    {item.hoca_name} · {formatAppointment(item.scheduled_start)}
                  </small>
                </span>
              </li>
            ))}
...
function HocaManagement({
  ...
  const setRole = async (id: string, role: "user" | "hoca") => {
          ...
          {users.map((item) => (
            <div className="admin-user-row" key={item.id}>
              ...
              {item.role !== "admin" && (
                ...
                >
                  {item.role === "hoca" ? "Yetkiyi kaldır" : "Hoca yap"}
                </button>
              )}
            </div>
          ))}
