...
export default function SahApp(){
  ...
  return <div className="core-app">
    ...
    {moreOpen&&<div className="mobile-more">{[...primaryNav.filter(i=>!['dashboard','journal','matrix'].includes(i.id)),{id:'reports' as ViewKey,label:'Raporlar',icon:'▥'}].map(item=><button key={item.id} onClick={()=>navigate(item.id)}><i>{item.icon}</i><span>{item.label}</span></button>)}<button onClick={()=>openPage('/feedback')}><i>✦</i><span>Görüş</span></button>{session.user.app_metadata?.role==='admin'&&<button onClick={()=>openPage('/admin/feedback')}><i>⌁</i><span>Yönetim</span></button>}</div>}
  </div>;
}
