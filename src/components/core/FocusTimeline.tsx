...
export default function FocusTimeline({ sessions, compact = false }: { sessions: FocusSession[]; compact?: boolean }) {
  const now = new Date()
  ...
  return <section className={`focus-timeline ${compact ? 'compact' : ''}`} aria-label="Bugünün odaklanma süresi kayıtları">
    <header><div><span className="eyebrow">BUGÜNÜN ODAK KAYITLARI</span><h2>Odaklanma akışın</h2></div><span className="focus-total-chip"><AppIcon name="clock-hour-4" /> {formatFocusDuration(total)}</span></header>
    <div className="focus-total-track" aria-label={`Bugün toplam ${formatFocusDuration(total)} odaklanıldı`}><i style={{ width: `${Math.min(100, total / (4 * 60 * 60) * 100)}%` }} /></div>
    <div className="focus-timeline-canvas" style={{ '--timeline-hours': String(Math.max(1, endHour - startHour)) } as React.CSSProperties}>
      ...
      {todaySessions.map((session, index) => {
        ...
        return <article key={session.id} className={`focus-session-block tone-${index % 4}`} style={{ top: `${top}%`, height: `${height}%` }} title={`${session.taskLabel} · ${formatFocusDuration(session.actualDurationSeconds)}`}>
          <strong>{session.taskLabel}</strong><span>{formatFocusDuration(session.actualDurationSeconds)}</span>
        </article>
      })}
