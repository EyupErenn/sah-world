...
function friendlyAuthError(error: unknown) {
  ...
  if (message.includes("provider") && message.includes("enabled"))
    return "Google ile giriş şu anda kullanılamıyor. E-posta koduyla devam edebilir veya daha sonra yeniden deneyebilirsiniz.";
  if (message.includes("rate") || message.includes("limit"))
    return "Çok sık deneme yapıldı. Lütfen kısa bir süre sonra tekrar deneyin.";
  if (message.includes("expired"))
    return "Kodun süresi dolmuş. Yeni bir kod isteyin.";
  if (message.includes("invalid"))
    return "Kod doğrulanamadı. Rakamları kontrol edip yeniden deneyin.";
...
export default function LoginScreen() {
  ...
  const [googleAvailability, setGoogleAvailability] =
    useState<GoogleAvailability>("checking");
  ...
  const handleGoogleLogin = async () => {
    ...
    const isInsecureDevelopmentHost =
    ...
    if (isInsecureDevelopmentHost) {
      ...
      setAuthError(
        "Google güvenlik doğrulaması yerel ağ adresinde çalışmaz. Aynı uygulamayı localhost üzerinden açıp yeniden deneyin.",
      );
    ...
    if (providerEnabled === false) {
      ...
      setAuthError(
        "Google girişi Supabase projesinde henüz etkin değil. Ayar tamamlanana kadar e-posta koduyla güvenle giriş yapabilirsiniz.",
      );
      setIsGoogleLoading(false);
      return;
    }
  ...
  const handleVerifyOtp = async () => {
    ...
    if (token.length !== 6)
      return setAuthError("6 haneli kodu eksiksiz girin.");
    setIsVerifying(true);
    setAuthError(null);
    try {
  ...
  const updateOtp = (index: number, value: string) => {
    ...
    setOtp((current) =>
      current.map((digit, itemIndex) => (itemIndex === index ? value : digit)),
    );
      ...
      <section className="login-story" aria-labelledby="login-title">
        ...
        <div className="login-story-copy">
          <h1 id="login-title">
            İyi alışkanlıkların,
            <br />
            <em>kendi evrenini kurar.</em>
          </h1>
          <p>
            Günlüğünü, hedeflerini ve manevi yolculuğunu tek bir sakin alanda
            takip et. Her kayıt ilerlemene dönüşsün.
          </p>
        </div>
        <CardioidMotif />
        ...
        <ul className="login-trust-list" aria-label="Platform özellikleri">
          ...
          <li>
            <span>✓</span> XH ve gelişim raporları
          </li>
          <li>
            <span>✓</span> Özel kayıtların sana ait
          </li>
        </ul>
      </section>
          ...
          {step === "email" ? (
            <>
              <div className="login-card-heading">
                ...
                <h2>SAH hesabına giriş yap</h2>
                <p>
                  İlerlemen güvenle saklansın ve tüm cihazlarında seninle
                  kalsın.
                </p>
              </div>
              <button
                ...
                disabled={
                  isGoogleLoading ||
                  isSending ||
                  googleAvailability === "checking"
                }
                aria-busy={isGoogleLoading || googleAvailability === "checking"}
              >
                <GoogleMark />
                <span>
                  {googleAvailability === "checking"
                    ? "Google girişi kontrol ediliyor…"
                    : isGoogleLoading
                      ? "Google’a yönlendiriliyor…"
                      : "Google ile devam et"}
                </span>
                {(isGoogleLoading || googleAvailability === "checking") && (
                  <Spinner />
                )}
              </button>
              {googleAvailability === "disabled" && (
                <p className="google-availability-note">
                  <span aria-hidden>i</span> Google bağlantısı yönetici ayarı
                  bekliyor. E-posta koduyla giriş kesintisiz çalışır.
                </p>
              )}
              <div className="login-separator">
                <span>E-posta kodu ile devam et</span>
              </div>
              ...
              <div className="login-email-row">
                <input
                  ...
                  onKeyDown={(event) =>
                    event.key === "Enter" && void handleSendOtp()
                  }
                />
                <button
                  type="button"
                  ...
                  aria-busy={isSending}
                >
                  {isSending ? <Spinner /> : "Kod gönder"}
                </button>
              </div>
              <p className="login-privacy-note">
                <span aria-hidden>⌁</span> Şifre istemiyoruz. Tek kullanımlık
                kod veya Google hesabınla güvenli giriş yaparsın.
              </p>
              {authError && (
            ...
            <>
              ...
              <div className="login-card-heading">
                <p className="eyebrow">Son bir adım</p>
                <h2>Gelen kutunu kontrol et</h2>
                <p>
                  <strong>{email}</strong> adresine gönderdiğimiz 6 haneli kodu
                  gir.
                </p>
              </div>
              ...
              <button
                className="sah-button-primary login-submit"
                type="button"
              ...
              <div className="login-resend">
                {countdown > 0 ? (
                  <span>
                    Yeni kodu {countdown} saniye sonra isteyebilirsin.
                  </span>
                ) : (
                  <button
                    type="button"
...
function GoogleMark() {
  return (
    <svg width="20" height="20" viewBox="0 0 18 18" aria-hidden>
      ...
      <path
        fill="#EA4335"
        d="M9 3.58c1.32 0 2.51.45 3.44 1.35l2.58-2.58A8.65 8.65 0 0 0 9 0 9 9 0 0 0 .96 4.96l3 2.33A5.4 5.4 0 0 1 9 3.58Z"
      />
    </svg>
  );
}
    ...
    <button
      ...
      onClick={() => {
        ...
        useAuthStore
          ...
          .setProfile({
            id: mockUser.id,
            display_name: "Geliştirme Misafiri",
            avatar_url: null,
            role: "user",
            quran_level: null,
            vehicle_type: "car",
