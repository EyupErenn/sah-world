...
export default function ProfileTab() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 animate-in slide-in-from-bottom-4 duration-500 fade-in select-none">
      ...
      {/* ============ HERO PROFILE CARD ============ */}
      <div className="glass-panel rounded-3xl p-6 md:p-8 flex flex-col md:flex-row items-center gap-8 relative overflow-hidden border border-white/10 shadow-2xl">
        ...
        <div className="relative flex-shrink-0">
          ...
          <div className="w-28 h-28 md:w-36 md:h-36 rounded-full border-4 border-indigo-400/50 bg-slate-950 overflow-hidden relative z-10 shadow-2xl">
            <img 
        ...
        <div className="flex-1 text-center md:text-left relative z-10 w-full">
          ...
          <div className="flex flex-wrap items-center justify-center md:justify-start gap-2.5 mb-5">
            <span className="glass-pill px-3 py-1 rounded-full text-indigo-300 text-xs font-bold flex items-center gap-1.5 border-indigo-500/30">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="glass-panel rounded-3xl p-6 border border-white/10 shadow-xl flex flex-col justify-between">
          ...
          {activity.every(a => a.count === 0) ? (
            <div className="h-32 flex flex-col items-center justify-center text-center p-4 rounded-2xl bg-white/[0.02] border border-dashed border-white/10">
                ...
                return (
                  <div key={i} className="flex-1 min-w-0 h-full flex flex-col items-center gap-2 group">
                    <div className="w-full flex-1 min-h-0 relative flex items-end justify-center">
                      ...
                      {/* Tooltip on Hover */}
                      <div className="absolute -top-9 bg-slate-900/90 text-white text-[11px] font-mono px-2.5 py-1 rounded-full opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none border border-white/15 whitespace-nowrap z-20 shadow-xl backdrop-blur-md">
                        {day.count} Amel
                      </div>
                    </div>
        ...
        <div className="glass-panel rounded-3xl p-6 border border-white/10 shadow-xl flex flex-col justify-between">
          <h3 className="text-sm font-bold text-slate-200 mb-5 flex items-center gap-2 tracking-wide uppercase">
...
function StatCard({ label, value, icon, color }: { label: string; value: string; icon: string; color: string }) {
  return (
    <div className="glass-card rounded-2xl p-4 flex flex-col items-center justify-center text-center hover:scale-[1.02] transition-all cursor-default">
