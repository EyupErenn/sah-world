-- Resolve production database linter findings in previously deployed helpers.
-- Function contracts and grants remain unchanged.
...
BEGIN
  ...
  IF target_kind = 'asma' THEN
    ...
    SELECT transliteration_turkish || ' tefekkürü'
      INTO label
      FROM public.asma_ul_husna
      WHERE order_number = target_reference_id::smallint;
  ELSE
    SELECT title || CASE WHEN lower(title) LIKE '%duası' THEN ' okundu' ELSE ' duası okundu' END
      INTO label
      FROM public.dua_library
      WHERE id = target_reference_id;
  END IF;

  IF label IS NULL THEN RAISE EXCEPTION 'spiritual_reference_not_found'; END IF;

  SELECT journal.id, journal.content
    INTO entry_id, current_content
    FROM public.journal_entries AS journal
    WHERE journal.user_id = actor AND journal.date = local_today
  ...
  IF entry_id IS NULL THEN
    INSERT INTO public.journal_entries(user_id, date, content, tags)
      VALUES (actor, local_today, '', ARRAY['mescidim'])
      RETURNING id, content INTO entry_id, current_content;
  END IF;

  INSERT INTO public.journal_spiritual_links(
    user_id, journal_entry_id, entry_date, entry_kind, reference_id,
    display_label, reflection_note, xp_awarded
  )
  SELECT actor, entry_id, local_today, target_kind, target_reference_id, label,
    nullif(left(btrim(reflection_text), 1000), ''),
    CASE WHEN (
      SELECT count(*)
      FROM public.journal_spiritual_links AS daily_link
      WHERE daily_link.user_id = actor
        AND daily_link.entry_date = local_today
        AND daily_link.xp_awarded > 0
    ) < 3 THEN 10 ELSE 0 END
  ON CONFLICT (user_id, entry_date, entry_kind, reference_id) DO NOTHING
  RETURNING id, journal_spiritual_links.xp_awarded INTO link_id, award;
  ...
  SELECT count(*)::integer
    INTO awarded_count
    FROM public.journal_spiritual_links AS counted_link
    WHERE counted_link.user_id = actor
      AND counted_link.entry_date = local_today
      AND counted_link.xp_awarded > 0;
